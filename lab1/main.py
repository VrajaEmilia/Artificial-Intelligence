...
class Drone():
    ...
    def moveDSF(self, detectedMap):
        # TODO!
        # rewrite this function in such a way that you perform an automatic mapping with DFS
        # DIRECTION order: UP RIGHT DOWN LEFT
        # mark the current square as visited
        self.visited.append([self.x,self.y])
        #up
        if self.isValid(detectedMap,self.x-1,self.y) and [self.x-1,self.y] not in self.visited:
            self.stack.append([self.x, self.y])
            self.stack.append([self.x-1,self.y])
        #right
        elif self.isValid(detectedMap,self.x,self.y+1) and [self.x,self.y+1] not in self.visited:
            self.stack.append([self.x, self.y])
            self.stack.append([self.x, self.y+1])
        #down
        elif self.isValid(detectedMap,self.x+1,self.y) and [self.x+1,self.y] not in self.visited:
            self.stack.append([self.x, self.y])
            self.stack.append([self.x+1, self.y])
        #left
        elif self.isValid(detectedMap,self.x,self.y-1) and [self.x,self.y-1] not in self.visited:
